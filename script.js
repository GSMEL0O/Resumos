document.addEventListener('DOMContentLoaded', () => {
    ...
    function saveSummary(subject, summary, name, file) {
        const summaries = JSON.parse(localStorage.getItem('summaries')) || [];
        summaries.push({ subject, summary, name, file });
    ...
    summaryForm.addEventListener('submit', (e) => {
        ...
        const name = document.getElementById('name').value.trim(); // Nome do aluno
        const fileInput = document.getElementById('file-upload');
        const file = fileInput.files[0] ? URL.createObjectURL(fileInput.files[0]) : '';

        if (subject && summary && name) {
            saveSummary(subject, summary, name, file);
            displaySummaries();
            summaryForm.reset();
        }
    });
